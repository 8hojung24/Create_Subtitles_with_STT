...
class CMultiMedia(QObject):
    ...
    def __init__(self, widget, video_widget):  
        ...
        self.list_2 = QMediaPlaylist()
        self.player.setPlaylist(self.list_2)
    ...
    def addMedia(self, files):
        for f in files:
            ...
            self.list_2.addMedia(QMediaContent(url))
            
    def addMedia2(self, files):
        for f in files:
            url = QUrl.fromLocalFile(f)
            self.list_2.addMedia(QMediaContent(url))   
    ...
    def delMedia2(self, index):
        self.list_2.removeMedia(index)
